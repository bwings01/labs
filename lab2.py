...
# Due Date: 01/26/2025
# Purpose: Calculate the amount of whole pizzas needed, salads needed, the total from the cost of whole pizzas and salads, the discount for whole pizzas and salads, the delivery fee, and the total amount due. Print out the the same values in an easy to read format.
...
def pizza_salad_order(pizza_orders, salad_orders):
    # calculation of pizza orders to show that a whole pizza is 12 slices but each order is 3 slices. the base price of a whole pizza is 15.99. calculation of the total slices needed for the order and the whole pizzas needed by dividing the total slices needed by the slices in a whoe pizza and rounding that to a base number. calculates the total cost of the pizzas by multiplying the amount of whole pizzas needed by the cost of a whole pizza
...
def order_up():
    ...
    # prints out each of the now defined variables with clear labels beside them to show the transaction total with all the the different parts of the total amount due
